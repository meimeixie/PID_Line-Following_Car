...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowIntNum(u16 x,u16 y,u32 num,u8 len,u8 size,u16 color)
{         	
	u8 t,temp;
	u8 enshow=0;						   
	for(t=0;t<len;t++)
...
//color:颜色 
//num:数值(0~4294967295);	 
void LCD_ShowFloatNum(u16 x,u16 y,float num,u8 lenINT,u8 lenDEC,u8 size,u16 color)
{         	
	u32 IntNum;//整数部分
	u32 DecNum;//小数部分
	LCD_ShowIntNum(x,y,(int32_t)num,lenINT,size,color);
	LCD_ShowString(x+lenINT*size/2,y,320,240,size,".",color);
	DecNum=((uint32_t)(num*LCD_Pow(10,lenDEC)))%LCD_Pow(10,lenDEC);
	LCD_ShowIntNum(x+(lenINT+1)*size/2,y,(int32_t)DecNum,lenDEC,size,color);
} 
//显示数字,高位为0,还是显示
//x,y:起点坐标
//num:数值(0~999999999);	 
